...
/* Enable center tap short */
static int ctap_short = 0;
module_param(ctap_short, int, 0);
MODULE_PARM_DESC(ledsel, "Enable center tap short");
			 ...
			 const struct usb_device_id *id)
{
	...
	tp->ups_info.ctap_short_off = !ctap_short;
...
#ifdef RTL8152_DEBUG
	sysfs_remove_group(&tp->netdev->dev.kobj, &rtk_dbg_grp);
#endif
	sysfs_remove_group(&tp->netdev->dev.kobj, &rtk_adv_grp);
