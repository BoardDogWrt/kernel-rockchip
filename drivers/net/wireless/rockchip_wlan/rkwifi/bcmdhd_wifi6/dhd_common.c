...
#if defined(DHD_DEBUG_TRACE_2)
int dhd_msg_level = DHD_ERROR_VAL | DHD_PKT_MON_VAL //
					| DHD_FWLOG_VAL | DHD_TRACE_VAL | DHD_INFO_VAL //
					| DHD_DATA_VAL | DHD_EVENT_VAL | DHD_PNO_VAL //
					| DHD_MSGTRACE_VAL | DHD_DNGL_IOVAR_SET_VAL //
					| DHD_IOVAR_MEM_VAL | DHD_CFG_VAL | DHD_CONFIG_VAL;
...
dhd_wl_ioctl(dhd_pub_t *dhd_pub, int ifidx, wl_ioctl_t *ioc, void *buf, int len)
{
	...
	if (dhd_os_proto_block(dhd_pub))
	{
		...
		if ((WLC_DISASSOC == ioc->cmd) || (WLC_DOWN == ioc->cmd) ||
			...
			DHD_TRACE(("IOCTL Disconnect WiFi: %d\n", ioc->cmd));
		}
		...
		if (DHD_BUS_CHECK_DOWN_OR_DOWN_IN_PROGRESS(dhd_pub)) {
			DHD_TRACE(("%s: returning as busstate=%d\n",
		...
		if ((ioc->cmd == WLC_GET_VAR || ioc->cmd == WLC_SET_VAR)) {
			if (buf != NULL) {
				...
				DHD_IOVAR_MEM(("%s: [%s] cmd: %d, len: %d, set: %d, txn-id: %d\n",
					__func__, ioc->cmd == WLC_GET_VAR ? "WLC_GET_VAR" : "WLC_SET_VAR",
			...
			if (buf != NULL && slen != 0) {
				...
				if (ioc->cmd != WLC_GET_MAGIC && ioc->cmd != WLC_GET_VERSION)
					DHD_IOVAR_MEM(("%s: cmd: %d, val: %d, len: %d, "
						"set: %d\n", __func__, ioc->cmd, val, ioc->len, ioc->set));
			} else {
				DHD_ERROR(("%s: cmd: %d, buf is NULL\n", __func__, ioc->cmd));
			}
		}
#endif /* DHD_LOG_DUMP */
		if (ret && dhd_pub->up) {
	...
	wl_event_msg_t *event, void **data_ptr, void *raw_event)
{
	...
	case WLC_E_SLOTTED_BSS_PEER_OP:
		DHD_EVENT(("%s: WLC_E_SLOTTED_BSS_PEER_OP event received for peer: "
			"" MACDBG ", status = %d\n",
			__func__, MAC2STRDBG(event->addr.octet), status));
...
dhd_pktfilter_offload_enable(dhd_pub_t * dhd, char *arg, int enable, int master_mode)
{
	...
	if (rc) {
		...
		__func__, enable?"enable":"disable", arg, rc));
		...
		__func__, enable?"enable":"disable", arg));
	...
	rc = rc >= 0 ? 0 : rc;
	if (rc)
		DHD_ERROR(("%s: failed to set pkt_filter_mode %d, retcode = %d\n",
			__func__, master_mode, rc));
	...
	if (!len) {
		...
		*length = 0;
		...
		if (file_path) {
			...
			if (image == NULL) {
				DHD_ERROR(("%s: Open image file failed %s\n", __func__, file_path));
				goto err;
			}
		}
...
dhd_check_current_clm_data(dhd_pub_t *dhd)
{
	...
	if (err == 0) {
		...
		DHD_ERROR(("%s: bcm_mkiovar failed.", __func__));
		return err;
	}
	...
	if ((strncmp(cspec->ccode, WL_CCODE_NULL_COUNTRY, WLC_CNTRY_BUF_SZ)) == 0) {
		DHD_TRACE(("%s: Firmware does not contain CLM data.\n", __func__));
		return FALSE;
	}
	DHD_TRACE(("%s: CLM is included in firmware\n", __func__));
	return TRUE;
}
...
dhd_apply_default_clm(dhd_pub_t *dhd, char *clm_path)
{
	...
	if ((len > 0) && (len < MAX_CLM_BUF_SIZE) && memblock) {
		...
		DHD_TRACE(("Skipping the clm download. len:%d memblk:%p \n", len, memblock));
	}
		...
		if (trailer->version == 1) {
			...
			if (hdr->trailer.fw_id != fwid) {
				DHD_ERROR(("%s: logstr id does not match FW!"
					...
					__func__, hdr->trailer.fw_id, fwid));
				goto error;
			}
...
dhd_start_event_ecounters(dhd_pub_t *dhd)
{
	...
	for (i = 0; i <= event_id_cnt; i++) {
		...
		for (j = id_array[i].str_idx; j < (uint8)ARRAYSIZE(event_ecounters_cfg_tbl); j++) {
			...
			if (rc) {
				DHD_ERROR(("%s: Could not process: stat: %d return code: %d\n",
					__func__, event_ecounter_stat->stats_rep, rc));
				goto fail;
			}
		...
		DHD_TRACE(("%s req version %d logset %d event_id %d flags %d len %d\n",
			__func__, req->version, req->logset, req->event_id,
			req->flags, req->len));
...
dhd_logdump_cookie_count(dhd_pub_t *dhdp)
{
	if (!dhdp || !dhdp->logdump_cookie) {
		DHD_ERROR(("%s: At least one buffer ptr is NULL dhdp=%p cookie=%p\n",
			__func__, dhdp, dhdp?dhdp->logdump_cookie: NULL));
		return 0;
	}
...
dhd_log_dump_cookie_to_file(dhd_pub_t *dhdp, void *fp, const void *user_buf, unsigned long *f_pos)
{
	...
	if (!dhdp || !dhdp->logdump_cookie || (!fp && !user_buf) || !f_pos) {
		DHD_ERROR(("%s At least one ptr is NULL "
			...
			__func__, dhdp, dhdp?dhdp->logdump_cookie:NULL, fp, f_pos));
		return ret;
	}
...
dhd_log_dump_trigger(dhd_pub_t *dhdp, int subcmd)
{
	...
	DHD_ERROR(("%s: current_time_sec=%lld debug_dump_time_sec=%lld interval=%d\n",
		__func__, current_time_sec, dhdp->debug_dump_time_sec,
	...
	if ((current_time_sec - dhdp->debug_dump_time_sec) < DEBUG_DUMP_TRIGGER_INTERVAL_SEC) {
		DHD_ERROR(("%s : Last debug dump triggered(%lld) within %d seconds, so SKIP\n",
			__func__, dhdp->debug_dump_time_sec, DEBUG_DUMP_TRIGGER_INTERVAL_SEC));
		return;
	}
		...
		void *evt_decode_data)
{
	...
	*/
	...
	if (ltoh16(msg->pyld_hdr.type) != PCIE_INFOBUF_V1_TYPE_LOGTRACE) {
		DHD_ERROR(("%s: payload_hdr_type %d is not V1_TYPE_LOGTRACE\n",
			__func__, ltoh16(msg->pyld_hdr.type)));
...
dhd_control_he_enab(dhd_pub_t * dhd, uint8 he_enab)
{
	...
	if (ret < 0) {
		DHD_ERROR(("%s he_enab (%d) set failed, err: %s\n",
			__func__, he_enab, bcmerrorstr(ret)));
	} else {
